...
- **ОБНОВЛЕННЫЕ ПРИОРИТЕТЫ (ИСПРАВЛЕНО С УЧЕТОМ ЗАВИСИМОСТЕЙ):**
  - VoiceRecognitionIntegration ✅ ЗАВЕРШЕНО (PTT работает)
  - ScreenshotCaptureIntegration ✅ ЗАВЕРШЕНО (скриншоты захватываются)
  - HardwareIdIntegration → КРИТИЧЕСКИЙ ПРИОРИТЕТ 1 (ОБЯЗАТЕЛЬНО для GrpcClient)
  - GrpcClientIntegration → ПРИОРИТЕТ 2 (зависит от Hardware ID)
  - SpeechPlaybackIntegration → ПРИОРИТЕТ 3 (зависит от GrpcClient)
...
1) ScreenshotCaptureIntegration (PROCESSING)
- Подписка: `app.mode_changed` → `mode=PROCESSING`
- Публикует: `screenshot.captured|error`
- Тест‑гейт: стабильный скриншот, корректная обработка отказа/разрешений

2) GrpcClientIntegration (PROCESSING)
- Вход: распознанный текст + скриншот
- Публикует: `grpc.request_started|completed|failed`
- Сеть: ретраи/бэкофф при `DISCONNECTED` (NetworkManager)
- Тест‑гейт: успешный ответ, корректное восстановление после потери сети

3) SpeechPlaybackIntegration (PROCESSING) — базовая
...
5) Workflows (формализация S→L→P→S)
- `listening_workflow.py`: дебаунс, отмены, таймауты LISTENING
- `processing_workflow.py`: capture → grpc → playback → SLEEPING, единая точка прерывания
...
#### **1.10 HardwareIdIntegration** 🔥 **КРИТИЧЕСКИЙ ПРИОРИТЕТ 1**
**Время:** 1-2 часа  
**Сложность:** Простая  
**Статус:** 🚀 **ГОТОВ К РЕАЛИЗАЦИИ**

**Почему ПЕРВЫЙ:** GrpcClient не может работать без hardware_id (обязательное поле в StreamRequest)

**Задачи:**
- [ ] Создать `HardwareIdIntegration` (тонкая обертка)
- [ ] Подключить к EventBus
- [ ] Получить hardware_id при запуске приложения
- [ ] Публиковать `hardware.id_obtained` с уникальным ID
- [ ] Кэширование ID для повторного использования

**Тест-гейт:** При запуске получен hardware_id, доступен для GrpcClient

#### **1.11 GrpcClientIntegration** 🔥 **КРИТИЧЕСКИЙ ПРИОРИТЕТ 2**
**Время:** 3-4 часа  
**Сложность:** Средняя  
**Статус:** 🚀 **ГОТОВ К РЕАЛИЗАЦИИ** (зависит от Hardware ID)

**Почему ВТОРОЙ:** Зависит от hardware_id, нужен для отправки данных на сервер

**Задачи:**
- [ ] Создать `GrpcClientIntegration` (тонкая обертка)
- [ ] Подключить к EventBus для получения данных
- [ ] Отправка (текст + скриншот + hardware_id) на Azure сервер
- [ ] Обработка стрим-ответов от сервера
- [ ] Ретраи и backoff при сетевых ошибках

**Тест-гейт:** Успешная отправка и получение ответа от Azure сервера

#### **1.12 SpeechPlaybackIntegration** 🔥 **КРИТИЧЕСКИЙ ПРИОРИТЕТ 3**
**Время:** 2-3 часа  
**Сложность:** Простая  
...
**ТЕКУЩИЙ СТАТУС:** 9 из 12 интеграций завершены! 🎉

**СЛЕДУЮЩАЯ ПОСЛЕДОВАТЕЛЬНОСТЬ:**
1. **Hardware ID Integration** (1-2 часа) - ОБЯЗАТЕЛЬНО для GrpcClient
2. **GrpcClient Integration** (3-4 часа) - отправка на Azure сервер  
3. **SpeechPlayback Integration** (2-3 часа) - завершение цикла

**Готов начать с Hardware ID Integration?** 🚀
