...
### 🔧 **СЛЕДУЮЩИЕ ШАГИ:**
1. **Добавить GitHub Secrets** (5 мин) - для автоматического деплоя
2. **Протестировать деплой** - сделать push в main
3. **Запустить упаковку PKG** - подпись + нотаризация
4. **Настроить AppCast** - update-server на порту 8081
